...
class User:
    ...
    def make_deposit(self, amount, account):	
        self.get_account(account).deposit(amount)
        return self

    def make_withdrawal(self,amount, account):
        self.get_account(account).withdrawal(amount)
        return self

    def display_user_balance(self, account):
        print(f"User: {self.name}, Account: {account.title()}, Balance: ${self.get_account(account).balance}")
